...
catsoop.modal = function(header, text, input, cancel){
	...
	if ('parentIFrame' in window) {
		...
		var top = x.scrollTop + x.clientHeight/2 - x.offsetTop - 200;
		if (top > (window.innerHeight - 400)){ top = window.innerHeight - 400; }
		if (top < 0){ top = 0; }
		// var top = window.innerHeight - x.scrollTop - x.clientHeight + 250;
		// console.log("[cs_ajax] st=", x.scrollTop, ", ch=", x.clientHeight, "wih=", window.innerHeight, ", oh=", x.offsetTop, ", top=", top);
